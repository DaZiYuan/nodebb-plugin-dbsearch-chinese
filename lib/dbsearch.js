...
// const tokenize = require('chinese-tokenizer').loadFile('./node_modules/nodebb-plugin-dbsearch-for-chinese/public/cedict_ts.u8')
const nodejieba = require("nodejieba");
nodejieba.load({
	userDict: './node_modules/nodebb-plugin-dbsearch-for-chinese/public/userdict.utf8',
});
...
function topicsSave(topics, callback) {
	...
	var data = topics.map(function (topicData) {
		var indexData = {};
		if (topicData.title) {
			indexData.content = tokenizeChinese(topicData.title);
		}
		if (topicData.cid) {
...
// 'chinese-tokenizer'的实现
// function tokenizeChinese(content) {
// 	content = tokenize(content);
// 	var result = "";
// 	Object.keys(content).forEach(function (key) {
// 		if (content[key])
// 			result += " " + content[key].text;
// 	});
...
function tokenizeChinese(text) {
	var result = nodejieba.cutAll(text);
	if (text.length < 500)
...
function postsSave(posts, callback) {
	...
	var data = posts.map(function (postData) {
		var indexData = {};
		if (postData.content) {
			indexData.content = tokenizeChinese(postData.content);
		}
		if (postData.cid) {
